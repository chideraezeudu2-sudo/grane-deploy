(function () {
  ...
  const CUSTOM_BUTTON_ICON = scriptTag && scriptTag.getAttribute('data-button-icon');

  // API URL: use explicit data-api-url if provided, otherwise use the page's origin
  // This ensures the widget works whether loaded from CDN or the app's own server
  const EXPLICIT_API_URL = scriptTag && scriptTag.getAttribute('data-api-url');
  const API_BASE = EXPLICIT_API_URL || window.location.origin;
  ...
  console.log('Grane monitoring active for app:', APP_ID);
  ...
  function createFeedbackButton() {
    ...
    // Default SVG icon (MessageCircle) - clean, recognizable
    ...
    btn.innerHTML = CUSTOM_BUTTON_ICON || defaultIcon;
    btn.style.cssText =
  ...
  function openFeedbackModal(type, context, eventId) {
    ...
    document.getElementById('apppulse-submit').onclick = function () {
      ...
      if (feedback && eventId) {
        fetch(`${API_BASE}/api/events/${eventId}/feedback`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
